...
class CloudProvider(NamedModel):
    ...
    provider_type: Mapped[CloudProviderType] = mapped_column(
        Enum(CloudProviderType), nullable=False, doc="Type of cloud provider"
    )

    region: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, doc="Default region for this provider"
    )

    credentials: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Encrypted credentials for the provider"
    )

    configuration: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Provider-specific configuration"
    )

    is_active: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, doc="Whether this provider is active"
    )
    ...
    connection_status: Mapped[str] = mapped_column(
        String(50), nullable=False, default="unknown", doc="Current connection status"
    )

    # Relationships
    infrastructures: Mapped[List["Infrastructure"]] = relationship(
        "Infrastructure", back_populates="cloud_provider", cascade="all, delete-orphan"
    )
...
class ResourceType(NamedModel):
    ...
    terraform_type: Mapped[Optional[str]] = mapped_column(
        String(200), nullable=True, doc="Terraform resource type identifier"
    )

    api_version: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, doc="API version for this resource type"
    )

    schema_definition: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="JSON schema for resource configuration"
    )

    default_configuration: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Default configuration values"
    )
    ...
    monitoring_config: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Monitoring and alerting configuration"
    )
...
class Infrastructure(NamedModel):
    ...
    environment: Mapped[str] = mapped_column(
        String(50), nullable=False, doc="Environment (dev, staging, prod, etc.)"
    )
    ...
    terraform_state: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Terraform state information"
    )

    configuration: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Infrastructure configuration"
    )

    variables: Mapped[Dict[str, str]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Terraform variables"
    )

    outputs: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Terraform outputs"
    )

    cost_estimate: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2), nullable=True, doc="Estimated monthly cost"
    )

    actual_cost: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2), nullable=True, doc="Actual monthly cost"
    )
    ...
    last_drift_check_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, doc="Last time drift detection was run"
    )
    ...
    cloud_provider: Mapped["CloudProvider"] = relationship(
        "CloudProvider", back_populates="infrastructures"
    )

    template: Mapped[Optional["InfrastructureTemplate"]] = relationship(
        "InfrastructureTemplate", back_populates="infrastructures"
    )
...
class InfrastructureResource(NamedModel):
    ...
    external_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, doc="External resource ID from the cloud provider"
    )

    region: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, doc="Cloud region where the resource is deployed"
    )
    ...
    configuration: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Resource configuration"
    )

    desired_configuration: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Desired resource configuration"
    )
    ...
    cost_per_hour: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 4), nullable=True, doc="Cost per hour for this resource"
    )
    ...
    infrastructure: Mapped[Optional["Infrastructure"]] = relationship(
        "Infrastructure", back_populates="resources"
    )
...
class InfrastructureTemplate(NamedModel):
    ...
    version: Mapped[str] = mapped_column(
        String(50), nullable=False, default="1.0.0", doc="Template version"
    )

    provider_type: Mapped[CloudProviderType] = mapped_column(
        Enum(CloudProviderType), nullable=False, doc="Target cloud provider"
    )

    category: Mapped[str] = mapped_column(
        String(100), nullable=False, doc="Template category (web-app, database, etc.)"
    )

    terraform_code: Mapped[str] = mapped_column(
        Text, nullable=False, doc="Terraform configuration code"
    )

    variables_schema: Mapped[Dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Schema for template variables"
    )

    default_variables: Mapped[Dict[str, str]] = mapped_column(
        JSON, nullable=False, default=dict, doc="Default variable values"
    )

    cost_estimate: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2), nullable=True, doc="Base cost estimate for this template"
    )
    ...
    # Relationships
    infrastructures: Mapped[List["Infrastructure"]] = relationship(
        "Infrastructure", back_populates="template", cascade="all, delete-orphan"
    )

    resource_types: Mapped[List["ResourceType"]] = relationship(
        "ResourceType", secondary="template_resource_types", back_populates="templates"
    )
